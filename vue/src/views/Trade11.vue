<template>
  ...
  <div style="display: flex; align-items: center;">
        <div style="margin: 150px 0px 110px 260px; background-color: rgb(55, 128, 224,0.2); width: 400px; height: 320px; padding: 20px; border-radius: 15px;">
            ...
            <el-form>
                <el-form-item>
                  <el-button disabled size="medium">当前股票价格：{{ price }}</el-button>
                </el-form-item>
                <el-form-item>
                  <el-button disabled size="medium">计划买入手数：{{ number }}(实际为 {{ number*100 }}支)</el-button>
                </el-form-item>
                <el-form-item>
                  <el-button disabled size="medium">当前该笔消费：{{ number*100*price }}</el-button>
                </el-form-item>
  ...
  methods: {
      ...
      {
        this.buy_para.id=this.user.id
        this.buy_para.username=this.user.user
        this.buy_para.stock_code=this.code
        this.buy_para.price=this.price
        this.buy_para.amount_in=this.number*100
        console.log(this.buy_para)
        

        this.request.post("transaction_in", this.buy_para).then(res => {
          if(res.code === 200) {
